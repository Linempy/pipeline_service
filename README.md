...
Зависимость `A -> B` означает, что узел `B` зависит от узла `A`. Поэтому в порядке выполнения `A` всегда расположен раньше `B`.
...
- Узел уникален по `nodeId` внутри одного пайплайна.
...
- Добавление уже существующей зависимости обрабатывается идемпотентно (граф не меняется, ошибка не выбрасывается.
...
Обоснование выбора: в ТЗ не заявлены требования к сохранности данных после перезапуска, совместной работе нескольких инстансов сервиса, истории изменений или сложным выборкам. PostgreSQL вместе с Docker Compose добавил бы миграции, конфигурацию и инфраструктурный код, но не улучшил бы эту центральную логику.

Плюсы выбранного подхода:

- сервис запускается одной командой без Docker и предварительной настройки;
- `ConcurrentHashMap` безопасен для конкурентного доступа к набору пайплайнов;
- операции над одним пайплайном синхронизированы, поэтому проверка и добавление ребра происходят атомарно.

Ограничение: данные находятся в памяти процесса и очищаются после перезапуска приложения. Для production-версии, если появится требование персистентности или горизонтального масштабирования, реализацию `PipelineRepository` можно заменить на PostgreSQL-репозиторий, не меняя контроллеры, сервисный слой и алгоритмы графа.

## Структура проекта

- `controller` — REST endpoints;
- `service` — бизнес-правила и синхронизация операций;
- `model` — доменная модель пайплайна, узлов и зависимостей;
- `repository` — абстракция и in-memory реализация хранения;
